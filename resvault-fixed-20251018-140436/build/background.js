...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    // Handle getPublicKey request
        ...
        (async function () {
            console.log('Handling getPublicKey action');
            console.log('Sender:', sender);
            let senderUrl = null;
            if (sender.tab && sender.tab.url) {
                senderUrl = sender.tab.url;
            ...
            // Try to get from sync storage first (logged-in user's main public key)
            chrome.storage.sync.get(['store'], async function (syncResult) {
                if (syncResult.store && syncResult.store.publicKey) {
                    const publicKey = syncResult.store.publicKey;
                    console.log('Returning public key from sync storage:', publicKey);
                    sendResponse({ 
                        success: true, 
                        ok: true,
                        publicKey: publicKey,
                ...
                // Fallback: Try local storage for domain-specific keys
                chrome.storage.local.get(['keys', 'connectedNets'], async function (result) {
                    const keys = result.keys || {};
                    const connectedNets = result.connectedNets || {};
                    const net = connectedNets[domain];
                    ...
                    if (keys[domain] && keys[domain][net]) {
                        ...
                        try {
                            ...
                            const decryptedPublicKey = await decryptData(publicKey.ciphertext, publicKey.iv, keyMaterial);
                            console.log('Returning decrypted public key:', decryptedPublicKey);
                            sendResponse({ 
                                success: true, 
                                ok: true,
                                publicKey: decryptedPublicKey,
                                pubkey: decryptedPublicKey
                            });
                        } catch (error) {
                            console.error('Error decrypting public key:', error);
                            sendResponse({ success: false, error: error.message });
                        }
                    } else {
                        console.error('No keys found for domain:', domain);
                        sendResponse({ 
                            success: false, 
    ...
    // Handle signMessage request
        ...
        (async function () {
            console.log('Handling signMessage action');
            console.log('Payload to sign:', request.payload);
            
            // For now, return an error explaining that signing isn't fully implemented
            // In a full implementation, this would use the private key from storage
            // and sign with Ed25519
            
            sendResponse({ 
                success: false, 
                error: 'Message signing is not yet fully implemented in this version of ResVault. Please upgrade the extension or use the login flow for authentication.'
            });
        })();
