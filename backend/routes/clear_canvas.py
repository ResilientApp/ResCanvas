...
def _persist_marker(id_value: str, value_field: str, value):
    """Persist a small marker into ResDB for recovery after Redis flush."""
    payload = {
        "operation": "CREATE",
        "amount": 1,
...
def submit_clear_canvas_timestamp():
    try:
        ...
        # Accept 'ts' or 'timestamp'; fallback to server time
        ts = _number(body.get('ts') or body.get('timestamp') or _now_ms())
        room_id = body.get('roomId') or body.get('room_id') or None

        # Determine current canvas draw-count (exclusive upper bound used by res-canvas-draw-<i>)
        try:
        ...
        # Persist markers to ResDB so Mongo mirror can be used for recovery
        try:
        ...
        try:
            if room_id:
                ...
                # delete per-stroke undo/redo markers that belong to this room
