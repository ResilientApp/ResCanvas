...
@require_auth
def create_room():
    return room_crud.create_room(rooms_collection, shares_collection, users_collection, notifications_collection, socketio)

@rooms_bp.route("/rooms", methods=["GET"])
@require_auth
def list_rooms():
    return room_crud.list_rooms(rooms_collection, shares_collection, users_collection)

@rooms_bp.route("/users/suggest", methods=["GET"])
@require_auth
def suggest_users():
    return room_crud.suggest_users(users_collection)

@rooms_bp.route("/rooms/suggest", methods=["GET"])
@require_auth
def suggest_rooms():
    return room_crud.suggest_rooms(rooms_collection, shares_collection, users_collection)

@rooms_bp.route("/rooms/<roomId>", methods=["GET"])
@require_auth
@require_room_access(room_id_param="roomId")
def get_room(roomId):
    return room_crud.get_room_details(rooms_collection, shares_collection)

@rooms_bp.route("/rooms/<roomId>", methods=["PATCH"])
@require_auth
@require_room_access(room_id_param="roomId")
def update_room(roomId):
    return room_crud.update_room(rooms_collection, shares_collection, notifications_collection, socketio)

@rooms_bp.route("/rooms/<roomId>/leave", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def leave_room(roomId):
    return room_crud.leave_room(rooms_collection, shares_collection, users_collection, notifications_collection, socketio)

@rooms_bp.route("/rooms/<roomId>", methods=["DELETE"])
@require_auth
@require_room_owner(room_id_param="roomId")
def delete_room(roomId):
    return room_crud.delete_room(rooms_collection, shares_collection, strokes_collection, invites_collection, notifications_collection, redis_client, socketio)

@rooms_bp.route("/rooms/<roomId>/admin/fill_wrapped_key", methods=["POST"])
@require_auth
def admin_fill_wrapped_key(roomId):
    return room_crud.admin_fill_wrapped_key(rooms_collection, users_collection)

@rooms_bp.route("/rooms/<roomId>/strokes", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def post_stroke(roomId):
    return room_strokes.post_stroke(rooms_collection, shares_collection, strokes_collection, users_collection, redis_client, socketio)

@rooms_bp.route("/rooms/<roomId>/strokes", methods=["GET"])
@require_auth_optional
def get_strokes(roomId):
    return room_strokes.get_strokes(rooms_collection, shares_collection, strokes_collection, users_collection, redis_client)

@rooms_bp.route("/rooms/<roomId>/clear", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def clear_room(roomId):
    return room_strokes.room_clear(rooms_collection, shares_collection, strokes_collection, redis_client, socketio)

@rooms_bp.route("/rooms/<roomId>/undo", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def undo(roomId):
    return room_undo_redo.room_undo(rooms_collection, shares_collection, strokes_collection, users_collection, redis_client, socketio)

@rooms_bp.route("/rooms/<roomId>/redo", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def redo(roomId):
    return room_undo_redo.room_redo(rooms_collection, shares_collection, strokes_collection, users_collection, redis_client, socketio)

@rooms_bp.route("/rooms/<roomId>/undo_redo_status", methods=["GET"])
@require_auth
@require_room_access(room_id_param="roomId")
def undo_redo_status(roomId):
    return room_undo_redo.get_undo_redo_status(redis_client)

@rooms_bp.route("/rooms/<roomId>/reset_my_stacks", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def reset_stacks(roomId):
    return room_undo_redo.reset_my_stacks(redis_client)

@rooms_bp.route("/rooms/<roomId>/share", methods=["POST"])
@require_auth
@require_room_access(room_id_param="roomId")
def share_room(roomId):
    return room_members.share_room(rooms_collection, shares_collection, users_collection, invites_collection, notifications_collection, socketio)

@rooms_bp.route("/rooms/<roomId>/members", methods=["GET"])
@require_auth
@require_room_access(room_id_param="roomId")
def get_members(roomId):
    return room_members.get_room_members(rooms_collection, shares_collection, users_collection)

@rooms_bp.route("/rooms/<roomId>/permissions", methods=["PATCH"])
@require_auth
@require_room_access(room_id_param="roomId")
def update_permissions(roomId):
    return room_members.update_permissions(rooms_collection, shares_collection, users_collection, notifications_collection, socketio)

@rooms_bp.route("/rooms/<roomId>/transfer", methods=["POST"])
@require_auth
@require_room_owner(room_id_param="roomId")
def transfer_ownership(roomId):
    return room_members.transfer_ownership(rooms_collection, shares_collection, users_collection, notifications_collection, socketio)
...
def delete_notification(nid):
    return invites_notifications.delete_notification(nid)
...
def notification_preferences():
    return invites_notifications.notification_preferences(notifications_collection)
